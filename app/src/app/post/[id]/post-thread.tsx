...
export function PostThread({ id, initial }: { id: string; initial: InitialThread | null }) {
  const router = useRouter();
  const { user } = useAuth();
  const [post, setPost] = useState<Post | null>(initial?.post ?? null);
  const [followups, setFollowups] = useState<Post[]>(initial?.followups ?? []);
  const [loading, setLoading] = useState(!initial);
  const [highlight, setHighlight] = useState<string | null>(null);
  const [openEditor, setOpenEditor] = useState<string | null>(() =>
    typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("echo"),
  );
